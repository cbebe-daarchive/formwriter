...
void setupButtons() {
  tft.drawRect(MAP_DISP_WIDTH, 0, 60, 320, TFT_RED);
  tft.drawRect(MAP_DISP_WIDTH, MAP_DISP_HEIGHT/2, 60, 320, TFT_RED);
  char slow[] = {'S','L','O','W'};
  char fast[] = {'F','A','S','T'};
...
void getRestaurantFast(int restIndex, restaurant* restPtr) {
  ...
  if (blockNum != blockNumFast) {
    while (!card.readBlock(blockNum, (uint8_t*) restBlock)) {
      Serial.println("Read block failed, trying again.");
    }
    blockNumFast = blockNum;
  }
  *restPtr = restBlock[restIndex % 8];
}
